...
class recoder():
    ...
    def reg(self):
        ...
        r = requests.post(url=self.Reg_url, data=json.dumps(
            RegData, sort_keys=True), headers=HTTP_HEADER)
    ...
    def define(self):
        ...
        self.Token_url = rospy.get_param('~REG_Token_url')
        # print 'self.Token_url',self.Token_url,type(self.Token_url)

        # default 'http://vop.baidu.com/server_api'
        self.Reg_url = rospy.get_param('~REG_Reg_url')
    ...
    def recode(self):
        pa = PyAudio()
        stream = pa.open(format=paInt16, channels=self.nchannel,
                         rate=self.SAMPLING_RATE, input=True, frames_per_buffer=self.NUM_SAMPLES)
        save_count = 0
        save_buffer = []
